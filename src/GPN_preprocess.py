...
def eval_seq(opt, dataloader, data_type, result_filename, evaluator, save_dir=None, show_image=True, frame_rate=30):
    if save_dir:
    ...
    for path, img, img0 in dataloader:
        ...
        online_targets, ghost_tracks = tracker.update(blob, img0, opt, evaluator, path)
        online_tlwhs = []
        online_ids = []
...
def main(opt, data_root='/data/MOT16/train', det_root=None, seqs=('MOT16-05',), exp_name='demo',
    ...
    for seq in seqs:
        ...
        if opt.vis_ghost_FP or opt.vis_ghost_FN:
            nf, ta, tc, plot_arguments = eval_seq(opt, dataloader, data_type, result_filename, evaluator,
                                  save_dir=output_dir, show_image=show_image, frame_rate=frame_rate)
        else:
            nf, ta, tc = eval_seq(opt, dataloader, data_type, result_filename, evaluator,
                                  save_dir=output_dir, show_image=show_image, frame_rate=frame_rate)
        n_frame += nf
